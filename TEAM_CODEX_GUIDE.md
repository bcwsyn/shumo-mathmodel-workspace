...
在仓库目录中打开 Codex，直接说“改进当前模型并验证创新”“根据已有结果完善摘要”“重做流程图并保留可编辑源”。Codex 根据根目录 `AGENTS.md` 和阶段技能执行，操作说明见 `docs/CONTENT_CAPABILITIES.md`。同步代码时一并同步项目 `reports/research_evidence.json` 和实际引用的报告、结果、图源；同一台账应由本轮负责整合的成员维护，合并冲突时逐项对照文件重新核验哈希。哈希变化需要复核内容，不能批量刷新后直接宣布通过。
