---
name: paper-writer
description: Supporting scientific-writing rulebook used by the installed survey and paper-reviewer skills. In this MathModel project, do not trigger it for mathematical-modeling papers; use 5writing for drafting and use this skill only as a referenced style/rule resource.
---

# Paper Writer

> Project adaptation: `5writing` remains authoritative for mathematical-modeling papers. This skill supplies supporting rules to `paper-reviewer` and must not replace G5/G6/G7.

A working-rules guide for writing scientific papers, distilled from John Martinis's *Notes on Writing a Scientific Paper* and Jan von Delft's *Style Guide*. Both source documents are preserved at `.agents/skills/paper-writer/references.md` and `.agents/skills/paper-writer/sources/` — consult them when this SKILL.md leaves a question open.
...
1. **Shared writing context.** Follow `.agents/skills/_shared/writing-workflow.md`. Use `$KB/NOTES.md` as the spine for prior work, gap statement, motivation, and conclusions.
...
- **Citations and missing references:** Follow `.agents/skills/_shared/writing-workflow.md`.
