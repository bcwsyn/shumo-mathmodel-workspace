...
- AI 可以协助提炼结构和发现视觉问题；正式图必须保留可编辑 SVG 或 Draw.io 主源，并在图示记录中如实说明工具使用及适用的披露要求。
...
然后从前序文档提取非数据图需求，先为每张候选图完成“图示论证合同”，再输出一个清单：
...
必须实际打开 PNG 或 PDF 进行视觉检查，确认文字清晰、箭头避让、模块对齐、留白、裁切和阅读顺序；发现问题要修主源并重新导出，不要只在报告里解释。
