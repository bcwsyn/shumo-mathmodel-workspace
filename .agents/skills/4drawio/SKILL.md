...
记录必须逐图说明：模块与证据定位、属性（数据事实/模型关系/决策偏好/情景假设）、AI 参与环节、可编辑主源和是否存在生成位图或水印。嵌入建议只说明每张图适合放入哪个章节和建议 caption，不生成 `*_typst_includes.typ`。最终的 Typst 图表插入代码由 `5writing` 根据论文结构决定。
