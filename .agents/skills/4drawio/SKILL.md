...
使用 `apply_patch` 创建或修改 `.drawio` 文件；不要在 PowerShell 中使用 Bash heredoc、`cat` 或 `mkdir -p`。生成大 XML 时分段提交补丁并在每次提交后验证 XML 完整性。最小结构示例：
...
配置路径必须真实存在。未配置时，Windows PowerShell 使用 `Get-Command drawio, draw.io -ErrorAction SilentlyContinue`，Unix shell 使用 `command -v`。必须记录绝对路径、进程退出码、PDF 大小和哈希；工具不存在时标记为 `UNVERIFIED`。如果无法导出 PDF，保留 `.drawio`，在 `reports/DRAWIO_REPORT.md` 记录失败原因和建议导出命令，不得虚构工具或静默宣称导出成功。
...
嵌入建议只说明每张图适合放入哪个章节和建议 caption，不生成 `*_typst_includes.typ`。最终的 Typst 图表插入代码由 `5writing` 根据论文结构决定。
