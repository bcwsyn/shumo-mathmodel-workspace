...
# Compatibility entry point. The workflow invokes writing_check.py directly
# with the project-confirmed interpreter; this wrapper is retained for users
# who explicitly launch the legacy shell command.
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PYTHON_BIN="${MATHMODEL_PYTHON:-python}"
...
# Naturalness and evidence-density review. These checks improve specificity
# and readability; they must never be used to conceal required AI disclosure.
filler_phrases = [
    "随着社会的不断发展",
    "随着科学技术的快速发展",
    "具有重要的理论意义和现实意义",
...
filler_hits = {phrase: paper_text.count(phrase) for phrase in filler_phrases if phrase in paper_text}
if sum(filler_hits.values()) >= 4:
    warn(f"high generic-phrase density; review for task-specific evidence: {filler_hits}")

plain_paragraphs = []
for paragraph in re.split(r"\n\s*\n", paper_text):
    plain = re.sub(r"#[A-Za-z][\w.-]*\s*\([^)]*\)", " ", paragraph)
...
for paragraph in plain_paragraphs:
    opening = re.sub(r"^[（(]?[一二三四五六七八九十0-9]+[）).、]?", "", paragraph)[:12]
    opening_map.setdefault(opening, 0)
    opening_map[opening] += 1
repeated_openings = {opening: count for opening, count in opening_map.items() if opening and count >= 3}
if repeated_openings:
    warn(f"repeated paragraph openings suggest mechanical structure: {repeated_openings}")

generic_claims = ["效果良好", "具有较高的准确性", "验证了模型的有效性", "具有较强的实用性", "具有广泛的应用前景"]
for claim in generic_claims:
    for sentence in re.findall(r"[^。！？!?]*" + re.escape(claim) + r"[^。！？!?]*[。！？!?]?", paper_text):
        if not re.search(r"\d|图\s*\d|表\s*\d|式\s*[（(]?\d", sentence):
            warn(f"generic conclusion lacks nearby numeric/figure/formula evidence: {sentence.strip()[:120]}")

sentence_candidates = [item.strip() for item in re.split(r"[。！？!?]", paper_text) if len(item.strip()) >= 25]
sentence_counts = {}
for sentence in sentence_candidates:
    normalized = re.sub(r"\s+", "", sentence)
    sentence_counts[normalized] = sentence_counts.get(normalized, 0) + 1
duplicates = [sentence[:60] for sentence, count in sentence_counts.items() if count >= 2]
if duplicates:
    warn(f"duplicate long sentences detected: {duplicates[:5]}")
