...
读取 `../research-evidence/SKILL.md` 与 `references/writing-figures.md`。G5 将章节论点对应已验证 claim 和图；G6 将摘要/核心结论中的数值对应成功实验指标、单位、舍入与正文实际定位；G7 登记绑定当前 PDF 的逐页视觉复核。分别运行该检查器 `--stage G5/G6/G7`（填本次实际阶段），同时保留本技能已有引用、文本、编译和视觉检查。局部改写仅更新受影响的台账和正文，不把“改摘要”扩展成模型重做。
