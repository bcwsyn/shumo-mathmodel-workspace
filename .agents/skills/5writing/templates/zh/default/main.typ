...
#set text(font: ("SimSun", "STSong", "Times New Roman"), size: 12pt, lang: "zh")
#set par(first-line-indent: (amount: 2em, all: true), justify: true, leading: 0.78em, spacing: 0.55em)
#set heading(numbering: "1.1")
#set enum(numbering: "1.")
#show heading.where(level: 1): it => block(above: 1.8em, below: 1.0em)[#align(center)[#text(font: ("SimHei", "SimSun"), size: 16pt, weight: "bold")[#it]]]
#show heading.where(level: 2): it => block(above: 1.25em, below: 0.65em)[#text(font: ("SimHei", "SimSun"), size: 14pt, weight: "bold")[#it]]
#show heading.where(level: 3): it => block(above: 0.9em, below: 0.4em)[#text(font: ("SimHei", "SimSun"), size: 12pt, weight: "bold")[#it]]
#show figure.caption: it => text(size: 9pt)[#it]

#let song = (body) => text(font: ("SimSun", "STSong"), body)
#let hei = (body) => text(font: ("SimHei", "SimSun"), weight: "bold", body)
#let kai = (body) => text(font: ("KaiTi", "STKaiti", "SimSun"), body)
#let tight-title(body, size: 18pt) = align(center)[#text(font: ("SimHei", "SimSun"), size: size, weight: "bold")[#body]]
#let keywords-cn(body) = block(above: 0.65em)[
  #set par(first-line-indent: 0pt)
  #text(font: ("SimHei", "SimSun"), weight: "bold")[关键词：]#body
]
#let running-header = [
...
#let toc-page(title: [目录]) = {
  align(center)[#text(font: ("SimHei", "SimSun"), size: 16pt, weight: "bold")[#title]]
  v(1em)
  outline(title: none)
  pagebreak()
}
...
#let three-line-table(caption, columns, header, body, inset: (x: 0.35em, y: 0.52em), cell-align: center) = {
  ...
  block(width: 100%, breakable: false)[
    #align(center)[
      #box[
        #align(center)[#text(font: ("SimHei", "SimSun"), size: 10.5pt, weight: "bold")[#caption]]
        #v(0.6em)
        #table(
          columns: columns,
          align: cell-align,
