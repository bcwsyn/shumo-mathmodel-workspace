...
先读取 `learning_project_contract.md`。本项目默认是 AI 主导的高质量完整建模模式：下文的建模、数值、写作和可复现性规范正常适用；竞赛名称只用于选择论文模板、语言、页面结构和格式样式。外部提交规则仅在用户明确要求直接参赛合规时另行核验。
...
| [CUMCM 当年规则核验](#cumcm-当年规则核验) | `1start-mathmodel` `5writing` `6verity` |
| [CUMCM 代码质量门禁](#cumcm-代码质量门禁) | `3coding-visual` `verify-generated-code` |
| [CUMCM 论文与支撑材料规范](#cumcm-论文与支撑材料规范) | `5writing` `6verity` |
| [高质量配图量化标准](#高质量配图量化标准) | `3coding-visual` `4drawio` `5writing` `6verity` |
...
- 论文不是工作日志。正文中不要出现内部文件名、脚本名、临时目录或审批流程说明。默认完整稿不生成外部提交披露材料；直接参赛模式仅处理用户明确指定的合规事项。
...
竞赛规则可能按年度修订。新项目默认借用对应 XeLaTeX 模板及其版式结构，不建立参赛合规报告；既有 Typst 或 Word 项目按既有格式维护，除非用户明确批准迁移。只有用户明确要求外部提交合规时，才另行建立合规报告，记录竞赛年份、正式文件、来源、获取日期、哈希、采用条款和核验状态。

正式材料按用户指定的赛事和年份单独核验。搜索结果摘要、培训材料、往年经验和模板默认值不能替代正式文件；不得把上一年度条款写成新项目事实。外部合规要求不写入通用建模规范，也不参与默认建模、实验、写作和最终质量验收。
## Python 代码质量门禁

先读取 `python_engineering_standard.md`，并以其为本项目代码质量的主契约。以下条款作为补充：

- 默认使用 `code/main.py` 单文件主实现，在文件内按数据读取、校验、预处理、模型、评价、约束检查、绘图和入口编排组织；公共逻辑只保留一个实现。
- 核心 API 和关键类使用规范 docstring；核心函数补全必要的参数与返回值类型标注，辅助函数不过度标注。
- 中文注释解释公式来源、变量单位、约束方向、数值稳定性和设计理由。避免逐行翻译语法、失效注释和用注释替代拆分。
- 配置、随机种子、输入和输出路径显式传入；不写死本机路径，不覆盖原始数据，不记录密钥。
- 单函数原则上不超过 50 行；确因算法连续性需要超过时记录理由并补充测试。
- 使用已确认解释器运行 `python -m ruff check code tests` 和 `python -m pytest -q tests`。工具缺失或未运行均不算通过。
- G2 至少覆盖正常小样本、边界输入和失败输入；G3 为各子问题保留确定性端到端小样本测试。
- 边界测试按题型覆盖空数据、缺失/重复、零值/负值、极端参数、不可行约束、错误形状、未知类别、时间边界等；断言具体数值、异常类型或约束状态。
- 测试不得只断言非空或“无异常”，不得通过调用被测实现重算自身期望值，也不得用 Mock 结果证明正式模型正确。

## 高质量配图量化标准

图表默认执行以下项目质量标准；只有用户明确启用外部提交合规时，再叠加其当年正式要求：

- 统计图、流程图和模型结构图优先输出 PDF/SVG 矢量格式，并保留可编辑源文件；无法矢量化的照片或栅格图按最终排版尺寸至少 300 DPI，纯线稿建议 600 DPI。
- 按论文实际栏宽设计。单栏图建议宽 80–90 mm，通栏图建议宽 160–180 mm；嵌入后避免二次大幅缩放。
- 最终有效字号原则上不低于 8 pt；坐标轴和图例通常使用 8–10 pt。主要线宽不低于 0.8 pt，关键曲线 1.2–1.8 pt，散点标记按密度使用约 3–6 pt。
- 全文统一中文/英文字体、数学字体、字号层级、线宽、标记、数值格式和色板；中文字体必须真实存在并正确嵌入或转曲，不接受缺字 fallback。
- 优先使用色盲友好色板，同时用线型、标记或明度提供冗余编码；灰度打印后仍能区分关键系列。避免彩虹色板、红绿单一编码和大面积高饱和色。
- 一张图只承担清晰论证任务。图内不写论文式大标题；坐标轴含名称和单位，图例短而明确，误差棒、置信区间和样本量按统计语义标注。
- 保存每张数据图对应的源数据、生成脚本、参数、随机种子和产物哈希，保证图中数值与结果表一一对应。
- 导出后逐图检查中文缺字、字体嵌入、坐标裁切、标签重叠、图例遮挡、异常留白、线条过细、颜色不可辨和数据遗漏；G3 前检查原图，G7/final 再检查论文中的最终排版尺寸。
- 图像只在必要时使用透明度和渐变；禁止装饰性 3D、无意义阴影、透视失真和会夸大差异的截断坐标轴。必须截断时清楚标识并解释。
...
- 灵敏度或稳健性分析原则上必须提供；确实不适用时需给出数学或实验理由。
...
默认只使用 MCM/ICM 模板、英文写作习惯和 Summary Sheet 结构。Summary Sheet 应凝练方法、创新点和关键结论。年度页数、附件和提交要求不写入通用规范；仅在用户明确启用外部合规时依据当年正式材料单独核验。
### 美赛评分关键维度
...
- 图表数量服从论证需要；优先用高信息密度图表解释模型结构、结果、误差、敏感性和稳健性，避免为凑数量堆图。
