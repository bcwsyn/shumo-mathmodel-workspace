...
#abstract-cn[
  [针对零配件缺陷、装配失效与售后调换共同作用的生产决策问题，本文构建抽样—生产—风险一体化模型。问题一采用精确单侧二项置信界和序贯概率比检验：22 件全合格时 90% 上界为 9.9372%，可接收；临界次品率附近平均检测量较高。问题二将检测、拆解和售后回收表示为吸收状态报酬过程，对 16 种策略比较。问题三用树形生产网络递推评估结构化候选策略，最佳候选期望净收益为 62.222222 元。问题四提出以抽样区间和 Beta 后验传播不确定性的框架；因题面未提供实际抽样记录，相关数值仅作情景分析。]
][
