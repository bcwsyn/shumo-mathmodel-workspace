...
def test_problem3_restricted_policy_comparison_has_expected_order() -> None:
    """Discarded internal failures must reacquire inputs, favoring their reuse."""
    best = main.all_inspected_tree_cost(True, False, True)
    discard = main.all_inspected_tree_cost(False, False, False)
    assert best == pytest.approx(60.2222222222)
    assert best > discard


def test_problem4_posterior_summary_is_finite_and_reproducible() -> None:
    """A small deterministic posterior run must report bounded policy stability."""
    first = main.posterior_problem2(main.SCENARIOS[1], 100, draws=20, seed=7)
    second = main.posterior_problem2(main.SCENARIOS[1], 100, draws=20, seed=7)
    assert first == second
    assert np.isfinite(first["mean_profit"])
    assert 0 <= first["optimal_frequency"] <= 1
