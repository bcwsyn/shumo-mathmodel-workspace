...
# 将 Matplotlib 缓存固定在项目内，避免只读用户目录导致不可复现的警告。
...
def tree_policy_profit(
    part_defects: tuple[float, ...],
    internal_defects: tuple[float, float, float],
    final_defect: float,
    dismantle_internal: bool,
    inspect_final: bool,
    dismantle_final: bool,
) -> float:
    """Evaluate one all-input-inspected policy for the table-2 assembly tree.

    A discarded failed assembly consumes its children again on the next attempt;
    a dismantled failed assembly preserves them and repeats only assembly-related
    costs.  Keeping those two renewal equations separate avoids undercounting
    the no-disassembly policies.
    """
    if len(part_defects) != 8:
        raise ModelInputError("problem-3 part_defects must contain eight rates.")
    for index, value in enumerate((*part_defects, *internal_defects, final_defect)):
        validate_probability(value, f"tree_defect_{index}")
    leaf_costs = [
        (cost + test) / (1 - defect)
        for cost, test, defect in zip(
            (2, 8, 12, 2, 8, 12, 8, 12),
            (1, 1, 2, 1, 1, 2, 1, 2),
            part_defects,
        )
    ...
    node_costs = []
    for group, defect in zip(((0, 1, 2), (3, 4, 5), (6, 7)), internal_defects):
        children = sum(leaf_costs[index] for index in group)
        if dismantle_internal:
            node_costs.append(children + (8 + 4 + defect * 6) / (1 - defect))
        else:
            node_costs.append((children + 8 + 4) / (1 - defect))
    children = sum(node_costs)
    if inspect_final:
        if dismantle_final:
            final_cost = children + (8 + 6 + final_defect * 10) / (1 - final_defect)
        else:
            final_cost = (children + 8 + 6) / (1 - final_defect)
    elif dismantle_final:
        final_cost = children + (8 + final_defect * (40 + 10)) / (1 - final_defect)
    else:
        final_cost = (children + 8 + final_defect * 40) / (1 - final_defect)
    return 200 - final_cost


def all_inspected_tree_cost(
    dismantle_internal: bool, inspect_final: bool, dismantle_final: bool
) -> float:
    """Evaluate the nominal table-2 tree within the stated candidate family."""
    return tree_policy_profit(
        (0.1,) * 8,
        (0.1, 0.1, 0.1),
        0.1,
        dismantle_internal,
...
def posterior_problem2(
    scenario: Scenario, sample_size: int, *, draws: int = 500, seed: int = 42
) -> dict[str, object]:
    """Propagate Jeffreys binomial uncertainty through all problem-2 policies."""
    if sample_size <= 0 or draws <= 0:
        raise ModelInputError("sample_size and draws must be positive.")
    nominal = np.asarray((*scenario.part_defects, scenario.assembly_defect))
    counts = np.rint(sample_size * nominal).astype(int)
    generator = np.random.default_rng(seed)
    rates = generator.beta(counts + 0.5, sample_size - counts + 0.5, size=(draws, 3))
    policies = all_policies()
    profits = np.full((draws, len(policies)), np.nan)
    for row, (part_1, part_2, assembly) in enumerate(rates):
        sampled = Scenario(
            (float(part_1), float(part_2)), scenario.part_costs,
            scenario.part_test_costs, float(assembly), scenario.assembly_cost,
            scenario.assembly_test_cost, scenario.sale_price,
            scenario.exchange_loss, scenario.disassembly_cost,
        )
        for column, policy in enumerate(policies):
            result = evaluate_policy(sampled, policy)
            if result.feasible:
                profits[row, column] = result.expected_profit
    valid = [column for column in range(len(policies)) if np.isfinite(profits[:, column]).all()]
    if not valid:
        raise RuntimeError("no feasible policy remained under posterior sampling.")
    means = np.asarray([profits[:, column].mean() for column in valid])
    chosen_column = valid[int(np.argmax(means))]
    draw_best = np.asarray(valid)[np.argmax(profits[:, valid], axis=1)]
    chosen_values = profits[:, chosen_column]
    return {
        "sample_size": sample_size,
        "draws": draws,
        "nominal_defect_counts": counts.tolist(),
        "recommended_policy": asdict(policies[chosen_column]),
        "mean_profit": float(chosen_values.mean()),
        "p05_profit": float(np.quantile(chosen_values, 0.05)),
        "optimal_frequency": float(np.mean(draw_best == chosen_column)),
    }


def posterior_problem3(
    sample_size: int, *, draws: int = 1_000, seed: int = 314
) -> dict[str, object]:
    """Propagate equal nominal 10% node-rate sampling uncertainty through table 2."""
    if sample_size <= 0 or draws <= 0:
        raise ModelInputError("sample_size and draws must be positive.")
    count = round(0.1 * sample_size)
    generator = np.random.default_rng(seed)
    rates = generator.beta(count + 0.5, sample_size - count + 0.5, size=(draws, 12))
    policies = list(itertools.product((False, True), repeat=3))
    profits = np.empty((draws, len(policies)))
    for row, sampled in enumerate(rates):
        for column, policy in enumerate(policies):
            profits[row, column] = tree_policy_profit(
                tuple(float(value) for value in sampled[:8]),
                tuple(float(value) for value in sampled[8:11]),
                float(sampled[11]),
                *policy,
            )
    means = profits.mean(axis=0)
    chosen = int(np.argmax(means))
    chosen_values = profits[:, chosen]
    return {
        "sample_size": sample_size,
        "draws": draws,
        "nominal_defect_count_per_node": count,
        "recommended_policy": {
            "internal_disassembly": policies[chosen][0],
            "final_inspection": policies[chosen][1],
            "final_disassembly": policies[chosen][2],
        },
        "mean_profit": float(chosen_values.mean()),
        "p05_profit": float(np.quantile(chosen_values, 0.05)),
        "optimal_frequency": float(np.mean(np.argmax(profits, axis=1) == chosen)),
    }
...
def run_posterior_stage() -> tuple[list[dict[str, object]], list[dict[str, object]]]:
    """Run problem-4 sample-size scenarios and draw their stability figure."""
    problem2_rows = [
        {
            "scenario": number,
            **posterior_problem2(
                scenario, sample_size, seed=1000 * sample_size + number
            ),
        }
        for sample_size in (100, 500)
        for number, scenario in SCENARIOS.items()
    ]
    problem3_rows = [posterior_problem3(size, seed=2000 + size) for size in (100, 500)]
    figure, axis = plt.subplots(figsize=(6.3, 3.6))
    positions = np.arange(1, 7)
    for offset, sample_size in ((-0.18, 100), (0.18, 500)):
        rows = [row for row in problem2_rows if row["sample_size"] == sample_size]
        axis.bar(
            positions + offset,
            [row["optimal_frequency"] for row in rows],
            width=0.34,
            label=f"每节点 n={sample_size}",
        )
    axis.set(xlabel="表 1 情形", ylabel="推荐策略成为逐次最优的后验频率", ylim=(0, 1.05))
    axis.set_xticks(positions)
    axis.legend()
    axis.grid(axis="y", alpha=0.25)
    save_figure(figure, "problem4_policy_stability")
    return problem2_rows, problem3_rows
...
def run_full() -> dict[str, object]:
    ...
    nominal_problem2 = []
    for number, scenario in SCENARIOS.items():
        best = evaluate_scenario(scenario)[0]
        nominal_problem2.append({"scenario": number, **serialise_policy_result(best)})
    posterior_problem2_rows, posterior_problem3_rows = run_posterior_stage()
    summary = {
        "sprt": sprt,
        "problem2_nominal": nominal_problem2,
        "problem2_sensitivity": sensitivity,
        "problem3_all_inspected_candidates": tree,
        "problem4_posterior": {
            "assumption": "nominal rates are rounded binomial counts under Jeffreys Beta(1/2,1/2) updating",
            "problem2": posterior_problem2_rows,
            "problem3": posterior_problem3_rows,
        },
        "seed": 42,
