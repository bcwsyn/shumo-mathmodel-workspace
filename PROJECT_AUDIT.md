...
历史完整项目（已从根目录归入 `projects/`，未删除）：

- projects/2021try1/
- projects/CUMCM2018A_HighTemperatureClothing/

历史交付包（已从根目录归入 `deliverables/`，未删除）：

- deliverables/交付包_生产质量决策模型_20260808/
...
- archive/incomplete-projects/B题_数学建模_20260808/：仅含输入与临时文件的不完整题目目录。
- archive/invalid-git-dir-20260823/：原根目录的空 `.git` 目录；它不是有效 Git 仓库。
...
原根目录的空 `.git` 不是有效 Git 仓库，现已可逆地移入 `archive/invalid-git-dir-20260823/`。当前目录仍不是 Git 仓库，因此不能用 git status 提供差异记录；本报告、CATALOG.md 和当前文件内容构成变更审计依据。
