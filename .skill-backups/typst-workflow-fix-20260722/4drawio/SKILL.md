---
name: 4drawio
description: "数学建模非数据型图示阶段。仅在 G3 结果批准后按需生成技术路线图、流程图和模型结构图，提交 G4 审批；也支持说明理由后由用户批准跳过。"
---
...
先读取 `.codex/runtime.local.json` 的 `drawio` 字段；未配置时再用当前平台的命令发现机制解析 DrawIO 绝对路径，然后导出 PDF：

```text
<drawio-absolute-path> --export --format pdf --crop --output figures/fig_roadmap.pdf figures/fig_roadmap.drawio
```

配置路径必须真实存在。未配置时，Windows PowerShell 使用 `Get-Command drawio, draw.io -ErrorAction SilentlyContinue`，Unix shell 使用 `command -v`。必须记录解析出的绝对路径、退出码和实际 PDF；工具不存在时标记为 `UNVERIFIED`。如果无法导出 PDF，保留 `.drawio`，在 `reports/DRAWIO_REPORT.md` 记录失败原因和建议导出命令，不得虚构工具或静默宣称导出成功。
