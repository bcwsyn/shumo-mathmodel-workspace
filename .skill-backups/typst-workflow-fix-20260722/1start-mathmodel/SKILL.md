...
最终验收与最终 PDF - `6verity` + `verify-generated-code`（仅在 G7 批准后）
```
...
- [ ] G7 审批：用户确认版式
- [ ] 最终验收、代码复验与最终 PDF - `6verity` + `verify-generated-code`
```
...
| 最终验收 | `6verity` + `verify-generated-code` | 仅在 G7 批准后复验轻量代码入口、核对正式产物并生成最终提交版。 | `VERIFY_REPORT.md`, final 审计报告, 最终 PDF |
