---
name: doctor
description: "检查数学建模工作流的实际运行环境并提供安装向导。用于验证用户指定、VS Code 选定、项目 .venv 或系统 Python 的绝对路径、包版本及 Typst、LaTeX、DrawIO、PDF 工具；仅在用户明确要求环境检查或安装时执行，安装前必须再次获得批准。"
---
...
脚本检查 Python、pip、numpy、scipy、pandas、matplotlib、scikit-learn、openpyxl、PyYAML，以及 typst、xelatex、drawio、pdftoppm、mutool、magick。外部工具优先读取 `.codex/runtime.local.json` 中同名字段，再回退到 PATH。报告必须包含解释器绝对路径、版本、每个工具的真实解析路径和发现来源。
...
- 论文阶段：按用户选择要求 Typst 或 xelatex；不要强制同时安装两者。
- DrawIO 仅在 G4 需要其导出时要求。
- PDF 栅格工具至少一个可用；若不可用，视觉检查能力降级并记录。
